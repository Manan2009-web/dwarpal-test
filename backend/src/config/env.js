...
const env = {
  ...
  brevoApiKeys: Array.from(
    new Set(
      [
        ...(process.env.BREVO_API_KEYS ? process.env.BREVO_API_KEYS.split(/[\n,;]+/) : []),
        ...
        process.env.BREVO_API_KEY_5
      ]
        .map((k) => normalizeEnvString(k))
        .filter(Boolean)
    )
  ),
  ...
  smtpConnectionTimeoutMs: parsePositiveIntegerEnv(process.env.SMTP_CONNECTION_TIMEOUT_MS, 10000),
  ...
  collegeName: (function() {
    const raw = normalizeEnvString(process.env.COLLEGE_NAME);
    return (raw && raw.toLowerCase() !== 'your college name') ? raw : 'Neotech Campus';
  })(),
