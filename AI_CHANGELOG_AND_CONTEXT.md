...
| :--- | :--- | :--- | :--- | :--- |
| **2026-09-19** | **Distinct & Precise Error Messaging (Separating Wrong Enrollment vs Wrong Password)** | 1. **Backend Separation**: In `authService.loginUser`, completely uncoupled user lookup failure from password comparison failure. When user is not found, throws `Enrollment number not found. Please check your enrollment number.` (`ERR_USER_NOT_FOUND`) with field error on `identifier`. When password does not match, throws `Incorrect password. Please check your password and try again.` (`ERR_INCORRECT_PASSWORD`) with field error on `password`.<br>2. **Frontend Preserved Field Errors**: In `App.jsx`, updated `resolveApiError`, `login`, and `handleLogin` to preserve `fieldErrors` and return them directly to the form so only the erroneous input field turns red with specific error text.<br>3. **Missing Field Precision**: In `handleLogin`, if only enrollment is blank, shows `Please enter your enrollment number.`; if only password is blank, shows `Please enter your password.`; only shows combined prompt if both are blank. | `backend/src/services/authService.js`<br>`src/App.jsx`<br>`src/lib/dwarpalApi.js`<br>`AI_CHANGELOG_AND_CONTEXT.md` | ✅ **Resolved** |
| **2026-09-19** | **Fix "Invalid enrollment number or password" issue occurring over time** | 1. **Zero-Width & Case-Insensitive Matching**: Enhanced `findUserByIdentifier` in `authService.js` to strip invisible unicode zero-width spaces (`\u200B-\u200D\uFEFF`), and perform case-insensitive regex & uppercase/lowercase indexed lookups across `enrollmentNo`, `enrollment`, `enrollmentNumber`, and `employeeId`. Added fallback lookup by registered phone number.<br>2. **Payload Extraction**: Updated `extractLoginIdentifier` to recognize `payload.enrollmentNo` and `payload.enrollmentNumber` (previously omitted).<br>3. **Cold-Start & Idle Reconnection**: Attached `dbConnectMiddleware` to `/auth/student-login-start`, `/auth/student-login-verify-otp`, `/auth/register`, and `/auth/forgot-password/*` routes in `authRoutes.js` to ensure the database connection is warm on Render/serverless cold starts before querying Mongoose.<br>4. **Temporary Enrollment Upgrade**: Updated `updateStudent` in `studentManagementService.js` to allow upgrading provisional student enrollment numbers to permanent GTU enrollment numbers when `student.isTemporaryEnrollment` is `true`.<br>5. **Input Sanitization & Error Transparency**: Updated `App.jsx` and `User.js` pre-validate hook to strip zero-width characters on paste/submit. Improved `resolveApiError` in `App.jsx` to preserve specific backend messages rather than blindly wiping every 401 with generic credential errors. | `backend/src/services/authService.js`<br>`backend/src/routes/authRoutes.js`<br>`backend/src/validators/authValidators.js`<br>`backend/src/services/studentManagementService.js`<br>`backend/src/models/User.js`<br>`src/App.jsx`<br>`AI_CHANGELOG_AND_CONTEXT.md` | ✅ **Resolved** |
| **2026-08-23** | **D-to-D Semester 3, Diploma Sem 1 & Degree Sem 1 Auto-Enrollment Support** | 1. Added full alias support for `D to D`, `D2D`, `D-to-D`, `Diploma to Degree`, `Direct Degree`, etc. to `PROGRAM_ALIASES` in both frontend and backend.<br>2. Configured auto-enrollment generation criteria for all 3 new student types: **Diploma Sem 1** (`2695903...`), **Degree Sem 1** (`2611701...`), and **D-to-D Sem 3** (`2611731...`).<br>3. Updated parsed Excel/CSV preview table to display `✨ Auto-Generate (New Student)` badge instead of error for missing enrollment on eligible new students.<br>4. Updated single student creation form with D-to-D auto-generation toggle. | `src/components/StudentManagementPanel.jsx`<br>`backend/src/utils/fieldNormalizer.js`<br>`backend/src/services/studentManagementService.js`<br>`backend/src/validators/adminStudentValidators.js`<br>`AI_CHANGELOG_AND_CONTEXT.md` | ✅ **Resolved** |
| **2026-08-23** | **Multi-Account Brevo Quota Monitor & Scrollable IT Notifications Feed** | 1. Built live Multi-Account Brevo Quota & Limit Monitor card in IT Email Management dashboard displaying all 6 accounts (`0/300 sent`, remaining credits, live status badge, and progress bar).<br>2. Implemented real-time live quota deduction via WebSockets when emails are sent by worker.<br>3. Implemented automatic verified sender resolution per account key.<br>4. Made the IT Notification feed container cleanly scrollable with custom slim scrollbar (`max-height: calc(100vh - 300px)`). | `src/components/StudentManagementPanel.jsx`<br>`src/App.css`<br>`backend/src/services/emailService.js`<br>`backend/src/controllers/adminController.js`<br>`backend/src/services/emailQueueService.js`<br>`AI_CHANGELOG_AND_CONTEXT.md` | ✅ **Resolved** |
